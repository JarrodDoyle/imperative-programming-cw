...
int convert_choice(const char choice[]) {
    ...
    else if (strcmp(choice, "Paper") == 0) return Paper;
    else if (strcmp(choice, "Scissors") == 0) return Scissors;
    return -1;
}
...
int main() {
    ...
    while (strcmp(input, "4")) {
        printf("\nWelcome to RPS\n 1. Best of 1\n 2. Best of 3\n 3. Best of 5\n 4. Quit\n\n");
        gets(input);
        if (strcmp(input, "1") == 0) play_game(1);
        else if (strcmp(input, "2") == 0) play_game(3);
        else if (strcmp(input, "3") == 0) play_game(5);
    }
    return 0;
}
