...
unsigned int colour(unsigned int r, int g, int b, int a) {
...
void coordinates(unsigned int p, int xyz[3]) {
    for (int i=0; i<3; i++) {
        int q = (p >> (20 - (i * 10))) & 0x3FF;
        xyz[i] = ((q & 0x200) != 0) ? q = (~0 << 10) | q : q;
    }
}
